export const TTS = (() => {
    ...
    function speak(text: string, options?: { lang?: string; rate?: number; pitch?: number; voiceName?: string }) {
        ...
        try {
            ...
            if (options?.voiceName) {
                const v = synth.getVoices().find((x) => x.name === options.voiceName || x.voiceURI === options.voiceName)
                if (v) u.voice = v
            }
            currentUtterance = u
            synth.speak(u)
            ...
            // noop
            // Some browsers throw if voices not loaded yet; try fallback
            try {
                const u = new SpeechSynthesisUtterance(text)
                currentUtterance = u
                synth.speak(u)
